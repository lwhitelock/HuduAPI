...
Context "Hudu Password Folders Integration Tests" {
    ...
    It "Creates, retrieves, updates, and deletes a series of generated password folders" {
        ...
        $passwordFoldersToCreate = $(get-Random -Minimum 4 -Maximum 8)
        $passwordFolderData = @{
            Requested = @()
        ...
        for ($i = 1; $i -le $passwordFoldersToCreate; $i++) {
            if ($(get-Random -Minimum 0 -Maximum 10) -gt 5) {
                ...
                $passFolderRequest= @{
                    Security = "specific"
                    AllowedGroups = (if $allgroups.count -gt 1) {$($allGroups | Get-Random -count $allgroups.count -1)} else {@(0)}
                }
            }
            ...
            $newPasswordFolder | Should -Not -BeNullOrEmpty
            ...
            if ($passFolderRequest.AllowedGroups) {
                $newPasswordFolder.AllowedGroups  | Should -Be $passFolderRequest.AllowedGroups            
            }
